 ...
 */
const renderMarkdownTextSimple = (
  text: string,
  forceWhite = false,
  keyPrefix = "txt",
): React.ReactNode => {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("**") && part.endsWith("**")) {
      return (
        <strong
          key={key}
          className="font-bold underline decoration-blue-500/20"
        >
          {renderMarkdownTextSimple(part.slice(2, -2), forceWhite, `${key}-b`)}
        </strong>
      );
    }
    if (part.startsWith("*") && part.endsWith("*")) {
      return (
        <i key={key} className="italic">
          {renderMarkdownTextSimple(part.slice(1, -1), forceWhite, `${key}-i`)}
        </i>
      );
    }
    if (part.startsWith("### ")) {
      return (
        <h3
          key={key}
          className="text-lg font-black mt-4 mb-2 text-blue-900 flex items-center gap-2 underline decoration-blue-500 text-left font-sans"
        >
          {part.slice(4)}
        </h3>
      );
    ...
    return (
      <span key={key} className={`${textColorClass} text-left font-sans`}>
        {part.replace(/<\/?[^>]+(>|$)/g, "")}
      </span>
    );
  });
...
const MarkdownTable: React.FC<{
  ...
  forceWhite?: boolean;
  idPrefix?: string;
}> = ({ markdown, forceWhite = false, idPrefix = "tbl" }) => {
  ...
  const rows = tableLines
    ...
    .filter(
      (row) => row.length > 1 && !row.every((cell) => cell.match(/^[ :\s-]*$/)),
    );
        ...
        <thead>
          ...
          >
            {rows[0].map((h, i) => (
              <th
                key={`${idPrefix}-th-${i}`}
                className="p-3 border-r border-slate-200 font-black uppercase tracking-tighter whitespace-nowrap"
              >
                {h}
              </th>
            ))}
    ...
    <div className="mt-16 bg-white rounded-[2.5rem] shadow-2xl border-2 border-slate-200 overflow-hidden text-left text-slate-800">
      <div className="bg-slate-900 p-8 flex items-center gap-4 text-white font-sans">
        ...
        <div className="text-left">
          <h3 className="font-black text-2xl tracking-tight leading-none font-sans">
            分析結果に対する質問
          </h3>
          <p className="text-sm text-sky-400 font-bold mt-2 leading-relaxed font-sans text-left">
            戦略深層ロジックについての質疑に応じます。
          </p>
        </div>
      </div>
      ...
      <div className="p-8 bg-slate-50/80">
        <div
          ref={scrollRef}
          className="h-[400px] overflow-y-auto mb-8 space-y-8 pr-4 custom-scrollbar text-left"
        >
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-slate-400 opacity-40">
              <Brain size={80} strokeWidth={1} />
              <p className="font-black text-xl mt-4 font-sans text-center">
                質問事項を記入して投稿してください。
              </p>
            </div>
          )}
          ...
          {messages.map((m) => (
            <div
              key={m.id}
              className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} animate-in fade-in slide-in-from-bottom-4 text-left`}
            >
              <div className="max-w-[90%] group">
                <div
                  className={`p-6 rounded-[2rem] shadow-sm ${m.role === "user" ? "bg-blue-600 text-white font-sans" : "bg-white border-2 border-slate-200 text-slate-800 font-sans"}`}
                >
                ...
                {m.role === "bot" && (
                  <div className="mt-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all text-left">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-white border rounded-full text-[10px] font-black shadow-sm active:scale-95 text-slate-800 font-sans"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-white border rounded-full text-[10px] font-black shadow-sm active:scale-95 text-amber-600 font-sans"
                    >
                      <StickyNote size={12} /> Keep
                    </button>
                    <button
                      onClick={() => handleAction(m.text, m.id, "share")}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-white border rounded-full text-[10px] font-black shadow-sm active:scale-95 text-blue-600 font-sans"
                    >
                      <Share2 size={12} /> 共有
                    </button>
                  </div>
          ...
          {loading && (
            <div className="flex justify-start items-center gap-3 p-4">
              ...
              <span className="font-black text-blue-600 uppercase animate-pulse font-sans">
                Strategic Scrutiny...
              </span>
            </div>
          )}
        ...
        <div className="relative group">
          ...
          <button
            ...
            className="absolute right-3 top-3 p-4 bg-blue-600 text-white rounded-[2rem] shadow-lg active:scale-90 hover:bg-blue-700 transition-all"
          >
            <Send size={24} />
          </button>
        </div>
...
const GenericTableParser: React.FC<{
  ...
  return (
    <div className="my-2 border-2 border-slate-300 rounded-[2.5rem] shadow-2xl overflow-hidden bg-white text-slate-900 min-h-[100px]">
      <div
        className={`p-6 flex items-center justify-between text-white ${colorClass || "bg-slate-900"}`}
      >
        <div className="flex items-center gap-4">
          <Icon size={26} className="text-white shrink-0" />
          <span className="font-black text-2xl tracking-tight uppercase leading-none font-sans text-left">
            {title}
          </span>
        </div>
        {isLoading && (
...
const AnalysisProgressBar: React.FC<{ progress: number; status: string }> = ({
  ...
  <div className="w-full max-w-4xl mx-auto mb-12 animate-in fade-in zoom-in duration-500">
    <div className="flex justify-between items-end mb-4 text-left">
      <div className="text-left">
        <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] mb-1 font-sans">
          Strategic Scanning Status
        </p>
        <div className="flex items-center gap-3">
          <Cpu className="text-blue-500 animate-pulse" size={20} />
          <h3 className="text-xl font-black text-white font-sans truncate max-w-[300px]">
            {status}
          </h3>
        </div>
      </div>
      <div className="text-right">
        <span className="text-3xl font-black text-blue-500 italic font-sans">
          {progress}%
        </span>
      </div>
    </div>
...
8. ## 長期的戦略ナラティブ（戦略目標、日本が発信すべき具体的ナラティブ、主導権掌握への効果）`;
  ...
  const handleWordExport = () => {
    const cleanForWord = (text: string) =>
    ...
    const convertToHtmlForWord = (md: string, title = "") => {
      ...
      let html = title
        ? `<p style="font-family:'Meiryo'; font-weight:bold; color:#1e3a8a; margin-top:20pt;">■ ${cleanForWord(title)}</p>`
        : "";
      ...
      if (tableLines.length >= 2) {
        ...
        tableLines.forEach((l, i) => {
          ...
          if (cells.length > 0) {
            const rowStyle =
              i === 0 ? "background-color:#eff6ff; font-weight:bold;" : "";
            html += `<tr style="${rowStyle}">`;
            cells.forEach((c) => {
              html += `<td style="border:1px solid #000; vertical-align:top;">${cleanForWord(c)}</td>`;
            });
            html += `</tr>`;
          }
        });
...
<body>
  ...
  const handleZohoHtmlExport = () => {
    const cleanForHtml = (text: string) =>
      ...
      if (tableLines.length >= 2) {
        ...
        tableLines.forEach((l, i) => {
          ...
          if (cells.length > 0) {
            ...
            cells.forEach((c) => {
              html += `<td style="padding:8px; vertical-align:top;">${cleanForHtml(c)}</td>`;
            });
            html += `</tr>`;
          }
        });
  ...
  const ActionButtons = () => (
    <div className="flex flex-wrap items-center gap-3">
      <button
        ...
        className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-black shadow-lg transition-all text-sm active:scale-95"
      >
      ...
      <div className="max-w-6xl mx-auto text-left font-sans">
        <header className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 pb-8 border-b border-slate-800 text-left font-sans">
          <div className="flex items-center gap-6 text-left font-sans">
            <div className="bg-white p-2 rounded-2xl shadow-2xl relative border-2 border-blue-500/30 shrink-0">
              ...
              <div className="absolute -bottom-1 -right-1 bg-blue-600 text-[8px] font-black px-1 rounded border border-white shadow-sm font-sans text-left">
                OSDSH
              </div>
            </div>
            <div className="text-left font-sans">
              <p className="text-xs font-black text-slate-500 tracking-widest uppercase font-sans">
                Strategic Analysis Engine (Type A)
              </p>
              <div className="flex items-baseline gap-3">
                <h1 className="text-3xl font-black text-white tracking-tighter uppercase leading-none font-sans">
                  中国複合法律戦 統合解析システム
                </h1>
                <span className="text-[10px] font-black bg-blue-600 text-white px-1.5 py-0.5 rounded leading-none shrink-0 shadow-sm border border-white/20 font-sans">
                  {SYSTEM_VERSION}
                </span>
              </div>
            </div>
          ...
          {showReport && (
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-white px-6 py-3 rounded-2xl font-black active:scale-95 transition-all text-sm border border-slate-600 shadow-lg font-sans"
              >
          ...
          <section className="bg-slate-900/50 p-6 md:p-12 rounded-[2.5rem] shadow-2xl border border-slate-800 backdrop-blur-xl animate-in fade-in duration-500 text-left font-sans">
            <div className="mb-8 p-6 bg-blue-950/40 border-l-[8px] border-blue-600 rounded-r-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex-1 text-left font-sans">
                <div className="flex items-center gap-2 mb-3 text-white font-sans text-left">
                  ...
                  <span className="font-black text-blue-400 text-xl tracking-tight uppercase">
                    タイプA 戦略解体・ナラティブ構築
                  </span>
                </div>
                <p className="text-slate-300 font-medium text-sm leading-relaxed text-left">
                  中国外交部会見等のテキストを貼り付けて解析を開始してください。
                </p>
              </div>
              <a
                ...
                className="flex items-center gap-3 bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all border border-blue-400 font-sans"
              >
                <Globe size={24} />
            ...
            <div className="mt-8">
              <button
                onClick={runAnalysis}
                disabled={!val.trim()}
                className="w-full py-6 rounded-[2rem] font-black text-xl bg-gradient-to-br from-blue-600 to-blue-800 text-white active:scale-95 transition-all flex items-center justify-center gap-4 border border-blue-400/30 shadow-2xl font-sans text-left px-12"
              >
        ...
        {loading && !showReport && (
          <div className="text-center py-40 flex flex-col items-center justify-center animate-in zoom-in duration-300 font-sans">
            ...
            <p className="text-3xl font-black text-blue-500 tracking-[0.2em] uppercase italic animate-pulse font-sans">
              Deconstructing Three-Wars...
            </p>
          </div>
        )}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20 text-left font-sans text-left">
            ...
            <div className="bg-[#fcfcfc] p-6 md:p-16 rounded-[2.5rem] shadow-4xl text-slate-900 border border-emerald-200 text-left font-sans">
              <div className="mb-12 border-b-2 border-slate-100 pb-8 text-left font-sans text-left">
                <h2 className="text-3xl font-black tracking-tighter text-slate-900 uppercase font-sans text-left">
                  中国複合法律戦 戦略解析レポート (Type A)
                </h2>
                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2 font-sans text-left text-left">
                  OSDSH DECONSTRUCTION SESSION: {dt}
                </p>
              </div>

              <div className="mb-16 text-left font-sans">
                <h2 className="text-2xl font-black text-blue-800 border-l-[10px] border-blue-800 pl-4 mb-8 flex items-center gap-3 font-sans text-left">
                  <InfoIcon /> 会見内容の総括
                </h2>
                <div className="p-8 bg-blue-50 border-2 border-blue-100 rounded-3xl mb-12 shadow-sm text-left">
                  {loading && !report.summaryHeader ? (
                    <div className="animate-pulse h-4 bg-blue-100 rounded w-full"></div>
                  ) : (
                    <div className="text-slate-700 font-bold leading-relaxed whitespace-pre-wrap font-sans text-left">
                      {renderMarkdownText(report.summaryHeader, false, "sh")}
                    </div>
                  )}
                </div>
                ...
                <h2 className="text-2xl font-black text-indigo-800 border-l-[10px] border-indigo-800 pl-4 mt-20 mb-8 flex items-center gap-3 font-sans text-left">
                  <Gavel /> 戦略深層メカニズム
                </h2>
                <GenericTableParser
                  content={report.mechanism}
                  title="三戦のからくり"
                  icon={Activity}
                ...
                <h2 className="text-2xl font-black text-blue-900 border-l-[10px] border-blue-900 pl-4 mt-20 mb-8 flex items-center gap-3 font-sans text-left">
                  <ShieldCheck /> 対抗戦略ナラティブ
                </h2>
                <GenericTableParser
                  content={report.tactical}
                  title="カウンター・ナラティブ案"
                  icon={Check}
