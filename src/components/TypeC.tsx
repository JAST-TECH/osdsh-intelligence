...
const parseDateString = (text: string, filename = ""): string => {
  ...
  const labelMatch = source.match(
    /(?:対象時期|時期|記者会見|日付).{0,15}?(\d{4})[\s年\.\-\/](\d{1,2})[\s月\.\-\/](\d{1,2})/,
  );
  if (labelMatch) {
    return `${labelMatch[1]}年${labelMatch[2].padStart(2, "0")}月${labelMatch[3].padStart(2, "0")}日`;
  }
  const yyyymmddMatch = source.match(
    /(?:_|^|\s|:)(20\d{2})([01]\d)([0-3]\d)(?:\b|$|_|\.)/,
  );
  if (yyyymmddMatch) {
...
const parseSessionName = (text: string, fallbackDate: string): string => {
  ...
  const infoLabelMatch = plain.match(
    /【(?:翻訳分析対象の)?情報名】\s*([\s\S]+?)(?:\n|■|###|【|$)/,
  );
  if (infoLabelMatch && infoLabelMatch[1].trim().length > 5)
    return infoLabelMatch[1].trim();

  const speakerMatch = plain.match(
    /(\d{4}年\d{1,2}月\d{1,2}日\s*外交部報道官.*?記者会見)/,
  );
...
const AppLogo: React.FC<{
  ...
  className?: string;
  isScanning?: boolean;
}> = ({ size = 60, className = "", isScanning = false }) => (
  <div
  ...
  >
    <svg width={size} height={size} viewBox="0 0 200 200">
      ...
      <path
        ...
        fill="url(#logoGradStableFinal)"
        className={isScanning ? "animate-spin origin-center" : ""}
...
const OSDSHLogo: React.FC<{ size?: number; className?: string }> = ({
  size = 100,
  className = "",
}) => (
  <div className={`flex flex-col items-center ${className}`}>
    ...
    >
      ...
      <path
        d="M100 20 L170 50 C170 120 100 180 100 180 C100 180 30 120 30 50 Z"
        fill="url(#shGradFinal)"
        stroke="#84cc16"
        strokeWidth="8"
      />
      <text
    ...
    <div className="text-[#84cc16] font-black text-[10px] mt-2 tracking-[0.1em] uppercase text-center font-sans max-w-[320px] leading-tight">
      {ENG_NAME}
    </div>
  </div>
);
        ...
        <thead>
          <tr className="bg-lime-50 text-lime-950 border-b-2 border-lime-200">
            {rows[0].map((h, i) => (
              <th
                key={i}
                className="p-4 border-r border-lime-100 font-black uppercase tracking-tighter text-left"
              >
                {h}
              </th>
            ))}
          ...
          {rows.slice(1).map((row, i) => (
            ...
            >
              {row.map((cell, j) => (
                <td
                  key={j}
                  className="p-4 align-top border-r border-slate-50 text-slate-800 leading-relaxed font-medium"
                >
                  {cell.split(/(\*\*.*?\*\*)/).map((p, k) =>
                    p.startsWith("**") ? (
                      <strong key={k} className="text-lime-800 font-bold">
                        {p.slice(2, -2)}
                      </strong>
                    ) : (
                      p
    ...
    <>
      {parts.map((part, i) => {
        ...
        return (
          <div
            key={i}
            className="whitespace-pre-wrap mb-4 text-left leading-relaxed"
          >
            {part.split(/(\*\*.*?\*\*)/).map((p, k) =>
              p.startsWith("**") ? (
                <strong key={k} className="text-lime-700 font-black">
                  {p.slice(2, -2)}
                </strong>
              ) : (
                p
      ...
      {isEditing ? (
        <div className="flex gap-1 bg-slate-800 p-1 rounded-lg border border-lime-500 animate-in zoom-in-95">
          <input
            ...
            value={date}
            onChange={(e) =>
              setDates((prev) => ({ ...prev, [`date${side}`]: e.target.value }))
            }
          />
          <button
            onClick={() =>
              setEditStates((prev) => ({ ...prev, [`edit${side}`]: false }))
            }
            className="text-lime-50 font-black px-1"
          >
            確定
          </button>
        </div>
        ...
        <div
          className={`px-3 py-1 rounded-full border ${date ? "border-lime-500/30 text-lime-400 bg-lime-500/10" : "border-red-500/30 text-red-400 bg-red-500/10"}`}
        >
          <Calendar size={10} className="inline mr-1" />
          {date || "日付未検出"}
          <button
            onClick={() =>
              setEditStates((prev) => ({ ...prev, [`edit${side}`]: true }))
            }
...
const TypeC: React.FC = () => {
  ...
  useEffect(() => {
    if (!editStates.editA)
      setDates((prev) => ({
        ...prev,
        dateA: parseDateString(inputs.typeA, inputs.fileA),
      }));
  ...
  useEffect(() => {
    if (!editStates.editB)
      setDates((prev) => ({
        ...prev,
        dateB: parseDateString(inputs.typeB, inputs.fileB),
      }));
  ...
  const generateUnifiedHtml = useCallback(() => {
    if (!report) return "";
    const clean = (t: string) =>
      sanitizeForDisplay(t).replace(/\*\*(.*?)\*\*/g, "<b>$1</b>");
    ...
    const tableToHtml = (md: string) => {
      const lines = md
        ...
        .filter((l) => l.includes("|"));
      if (lines.length < 2)
        return `<p style="font-size:11pt; color:#1e293b; text-align:left;">${clean(md)}</p>`;
      ...
      rows.forEach((row, i) => {
        ...
        html += `<tr style="${bg}">`;
        row.forEach((cell) => {
          html += `<td style="border:1px solid #cbd5e1; vertical-align:top; text-align:left;">${cell.replace(/\*\*(.*?)\*\*/g, "<b>$1</b>")}</td>`;
        });
        html += `</tr>`;
      });
      return html + `</table>`;
  ...
  const runAnalysis = async () => {
    if (!dates.dateA || !dates.dateB || dates.dateA !== dates.dateB) {
      setError(
        `日付の不一致または未検出です。(A: ${dates.dateA || "なし"} / B: ${dates.dateB || "なし"}) 手動で日付を修正してください。`,
      );
      return;
    }
      ...
      {error && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-slate-900 border-2 border-red-500 rounded-[2.5rem] p-10 max-w-lg w-full text-center shadow-4xl text-left">
            ...
            <p className="text-white font-black text-lg mb-8 whitespace-pre-wrap">
              {error}
            </p>
            <button
              onClick={() => setError(null)}
              className="w-full py-4 bg-red-500 hover:bg-red-600 rounded-2xl font-black text-white transition-all shadow-lg active:scale-95 uppercase tracking-widest"
            >
              閉じる
            </button>
          </div>
      ...
      <div className="max-w-6xl mx-auto text-left">
        <header className="mb-12 pb-10 border-b border-slate-800 text-left space-y-6">
          <div className="flex items-center gap-6 text-left">
            ...
            <div className="text-left flex-1">
              <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase leading-none text-left bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent font-sans">
                中国外交部記者会見
            ...
            {report && !loading && (
              <button
                ...
                className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 px-6 py-3 rounded-2xl font-black text-sm transition-all border border-slate-700 shadow-lg active:scale-90"
              >
          ...
          <div className="flex flex-col space-y-2 border-l-4 border-lime-500 pl-6 text-left">
            <p className="text-[13px] text-lime-400 font-black tracking-[0.2em] uppercase leading-tight text-left font-sans">
              {ENG_NAME}
            </p>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em] text-left">
              Sovereignty Defense Intelligence Core | {SYSTEM_VERSION}
            </p>
          </div>
        </header>
        ...
        {!report && !loading && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 text-left">
            {[
              { id: "A", label: "戦略解析データ (Type A)", color: "lime" },
              {
                id: "B",
                label: "個別影響分析データ (Type B)",
                color: "emerald",
              },
            ].map((side) => (
              <div key={side.id} className="space-y-4 text-left">
                <div className="flex justify-between items-center px-2 text-left">
                  <label
                    className={`text-${side.color}-400 font-black text-xs uppercase tracking-widest text-left`}
                  >
                    {side.label}
                  </label>
                  <DateDisplay
                    side={side.id as "A" | "B"}
                    date={dates[`date${side.id}` as keyof DateState]}
                    setDates={setDates}
            ...
            <button
              onClick={runAnalysis}
              className="md:col-span-2 py-8 bg-gradient-to-r from-lime-600 to-emerald-600 hover:from-lime-500 hover:to-emerald-500 rounded-[2.5rem] font-black text-2xl shadow-3xl active:scale-[0.98] transition-all flex items-center justify-center gap-4 border-2 border-white/10 uppercase tracking-widest"
            >
        ...
        {loading && (
          <div className="py-52 text-center animate-pulse flex flex-col items-center">
            ...
            <p className="mt-10 text-2xl font-black text-lime-500 uppercase tracking-[0.3em] italic text-center">
            ...
            <p className="mt-4 text-slate-500 font-bold text-xs uppercase tracking-widest">
              {statusMsg}
            </p>
          </div>
        )}
            ...
            <div className="bg-white p-10 md:p-20 rounded-[4.5rem] text-slate-900 shadow-5xl border border-slate-100 text-left relative overflow-hidden">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-8 border-lime-500 pb-12 mb-14 text-left w-full gap-8 text-left">
                ...
                <div className="text-left flex-1 text-left">
                  <h2 className="text-4xl font-black tracking-tighter uppercase text-slate-900 leading-none text-left mb-2 font-sans">
                    ナラティブ侵略解析サマリー
                  </h2>
                  <p className="text-lime-600 font-black text-[13px] tracking-[0.2em] uppercase text-left leading-none font-sans">
                    Intelligence Target Session Date: {dates.dateB}
                  </p>
                </div>
                <div className="flex gap-4 text-left">
                  <button
                    ...
                    title="Word保存"
                    className="bg-slate-950 text-white p-5 rounded-3xl hover:bg-slate-800 transition-all shadow-xl active:scale-90 border-2 border-white/10"
                  >
                    <Download size={24} />
                  </button>
                  <button
                    ...
                    title="HTMLコピー"
                    className="bg-lime-600 text-white p-5 rounded-3xl hover:bg-lime-500 transition-all shadow-xl active:scale-90 border-2 border-white/10"
                  >
                    <Code size={24} />
                  </button>
                </div>
              ...
              <div className="bg-slate-950 text-white p-12 rounded-[3rem] mb-20 border-l-[24px] border-lime-50 shadow-4xl flex items-center gap-10 text-left animate-in zoom-in-95 duration-500 text-left">
                <div className="bg-lime-500/10 p-4 rounded-3xl border border-lime-500/20 shadow-inner">
                  <FileSearch className="text-lime-400" size={48} />
                </div>
                <div className="text-left flex-1 space-y-1 text-left">
                  <span className="text-[12px] font-black text-lime-500 uppercase tracking-[0.3em] mb-2 block text-left opacity-80 font-sans text-left">
                    Analysis Target session
                  </span>
                  <h4 className="text-2xl md:text-3xl font-black leading-tight text-left tracking-tight font-sans text-left">
                    {report.infoName}
                  </h4>
                </div>
              </div>
              ...
              <div className="space-y-24 text-left">
                <section className="text-left">
                  <h3 className="text-3xl font-black text-lime-950 border-l-[16px] border-lime-50 pl-8 mb-10 flex items-center gap-5 text-left font-sans text-left">
                    <InfoIcon size={32} className="text-lime-600" /> 本日の核心
                  </h3>
                ...
                <section className="text-left animate-in slide-in-from-left duration-700 text-left">
                  <h3 className="text-3xl font-black text-red-900 border-l-[16px] border-red-600 pl-8 mb-10 flex items-center gap-5 text-left font-sans text-left">
                  ...
                  <div className="p-12 bg-red-50/50 border-4 border-red-200 border-dashed rounded-[3.5rem] text-[18px] font-medium text-slate-800 leading-[1.8] text-left shadow-lg font-sans text-left">
                    <p className="font-black text-red-800 mb-6 underline text-2xl tracking-tighter text-left">
                      【ナラティブ侵略の構造解体分析】
                    </p>
                ...
                <section className="text-left text-left">
                  <div className="bg-emerald-900 text-white p-10 rounded-t-[3.5rem] font-black text-2xl uppercase tracking-widest flex items-center gap-5 shadow-2xl text-left font-sans text-left">
                    <ShieldAlert size={36} /> 核心リスク・マトリクス
                  </div>
                ...
                <section className="text-left text-left">
                  <div className="bg-lime-800 text-white p-10 rounded-t-[3.5rem] font-black text-2xl uppercase tracking-widest flex items-center gap-5 shadow-2xl text-left font-sans text-left">
                    <Network size={36} /> 沖縄・南西諸島への直接波及
                  </div>
                ...
                <section className="text-left text-left">
                  <div className="bg-slate-900 text-white p-10 rounded-t-[3.5rem] font-black text-2xl uppercase tracking-widest flex items-center gap-5 shadow-2xl text-left font-sans text-left text-left">
                    <BrainCircuit size={36} /> 即時カウンター・ナラティブ案
                  </div>
                  <div className="p-8 bg-slate-50 border-x-2 border-b-2 border-slate-100 rounded-b-[3.5rem] text-left shadow-sm text-left font-sans text-left">
              ...
              <footer className="mt-32 pt-14 border-t-4 border-slate-100 text-left text-left">
                <div className="flex flex-col gap-6 text-left">
                  <div className="space-y-1 text-left text-left">
                    <p className="text-slate-950 font-black text-lg text-left font-sans text-left">
                      発行者：{PUBLISHER}
                    </p>
                    <p className="text-slate-600 font-bold text-sm text-left font-sans text-left">
                      ツール開発：{DEVELOPER}
                    </p>
                  </div>
                  <div className="pt-8 border-t border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4 text-left">
                    <span className="text-[12px] text-slate-400 font-black tracking-[0.4em] uppercase text-left font-sans text-left">
                      © 2026 OSDSH INFRASTRUCTURE
                    </span>
                    <span className="text-[11px] text-slate-300 font-bold tracking-widest uppercase italic text-left font-sans text-left">
                      {ENG_NAME}
                    </span>
                  </div>
                </div>
