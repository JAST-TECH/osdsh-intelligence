...
import { 
  Download, MessageSquare, Send, RefreshCw, Globe, 
  Info as InfoIcon, Check, Copy, ArrowLeft, Brain, 
  StickyNote, Loader2, Share2, Search, List, BookOpen, Code
} from 'lucide-react';
...
const AppLogo: React.FC<{ size?: number; className?: string; id?: string; isScanning?: boolean }> = ({ size = 60, className = "", id = "", isScanning = false }) => (
  <div className={`relative ${className}`} style={{ width: size, height: size }}>
    ...
    >
      ...
      <circle cx="100" cy="100" r="90" fill="white" stroke="#064e3b" strokeWidth="8" />
      <path d="M100 30 L130 90 L190 100 L130 110 L100 170 L70 110 L10 100 L70 90 Z" fill="url(#logoGradGreen)" />
      <circle cx="100" cy="100" r="25" fill="white" />
      <path d="M85 100 L115 100 M100 85 L100 115" stroke="#064e3b" strokeWidth="5" strokeLinecap="round" />
    </svg>
  </div>
);
 ...
 */
const renderMarkdownText = (text: string | undefined, keyPrefix = "txt"): React.ReactNode => {
  if (!text) return null;
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={key} className="font-bold">{renderMarkdownText(part.slice(2, -2), `${key}-b`)}</strong>;
    }
    if (part.startsWith('[赤]') && part.endsWith('[/赤]')) {
      return <span key={key} className="text-red-600 font-black">{part.slice(4, -5)}</span>;
    }
  ...
  return (
    <div className="mt-16 bg-white rounded-[2.5rem] shadow-2xl border-2 border-emerald-200 overflow-hidden font-sans">
      <div className="bg-emerald-900 p-8 flex items-center gap-4 text-white">
        <div className="p-3 bg-green-600 rounded-2xl shadow-lg"><MessageSquare size={28} /></div>
        <div>
          <h3 className="font-black text-2xl tracking-tight leading-none">分析結果への質問</h3>
          <p className="text-sm text-emerald-400 font-bold mt-2 leading-relaxed">最下段のボックスに質問を投稿してください。</p>
        </div>
      </div>
  
      <div className="p-8 bg-emerald-50/30">
        <div ref={scrollRef} className="h-[400px] overflow-y-auto mb-8 space-y-8 pr-4 custom-scrollbar">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-emerald-200 opacity-40 text-center">
              <Brain size={80} strokeWidth={1} /><p className="font-black text-xl mt-4">戦略的解析のための質問を入力してください。</p>
            </div>
          )}
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-4`}>
              <div className="max-w-[90%] group">
                <div className={`p-6 rounded-[2rem] shadow-sm ${m.role === 'user' ? 'bg-green-600 text-white' : 'bg-white border-2 border-emerald-200 text-slate-800'}`}>
                   <div className="text-base whitespace-pre-wrap font-medium leading-relaxed">{m.text}</div>
                </div>
                {m.role === 'bot' && (
                  <div className="mt-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    <button onClick={() => handleAction(m.text, m.id, 'copy')} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border rounded-full text-[10px] font-black shadow-sm active:scale-95 text-emerald-800">
                      {actionStatus.id === m.id && actionStatus.type === 'copy' ? <Check size={12} className="text-green-600"/> : <Copy size={12}/>} コピー
                    </button>
                    <button onClick={() => handleAction(m.text, m.id, 'keep')} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border rounded-full text-[10px] font-black shadow-sm active:scale-95 text-amber-600"><StickyNote size={12}/> Keep</button>
                    <button onClick={() => handleAction(m.text, m.id, 'share')} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border rounded-full text-[10px] font-black shadow-sm active:scale-95 text-emerald-600"><Share2 size={12}/> 共有</button>
                  </div>
                )}
              </div>
            </div>
          ...
          {loading && (
            <div className="flex justify-start items-center gap-3 p-4">
              ...
              <span className="font-black text-green-600 uppercase animate-pulse text-xs">Analyzing Intelligence...</span>
            </div>
          )}
        </div>
        <div className="relative group">
          <input 
            ...
            placeholder="質問を入力..." 
          />
          <button onClick={handleSend} disabled={loading || !input.trim()} className="absolute right-3 top-3 p-4 bg-green-600 text-white rounded-[2rem] shadow-lg active:scale-90 hover:bg-green-700 transition-all"><Send size={24} /></button>
        </div>
      </div>
    </div>
  );
 ...
 */
const GenericTableParser: React.FC<{ rows: TableRow[]; title: string; icon: React.ElementType; colorClass?: string; isLoading?: boolean }> = ({ rows, title, icon: Icon, colorClass, isLoading = false }) => {
  return (
    <div className="my-8 border-2 border-emerald-300 rounded-[2.5rem] shadow-2xl overflow-hidden bg-white text-slate-900 min-h-[100px]">
      <div className={`p-6 flex items-center justify-between text-white ${colorClass || 'bg-emerald-900'}`}>
        <div className="flex items-center gap-4">
          <Icon size={26} className="text-white shrink-0" />
          <span className="font-black text-2xl tracking-tight uppercase leading-none">{title}</span>
        </div>
        ...
        <table className="w-full text-[12px] border-collapse table-fixed min-w-[900px]">
          <thead>
            <tr className="bg-emerald-100 text-emerald-900 border-b-2 border-emerald-200">
              ...
              <th className="p-4 text-left font-black w-[30%]">中国語 (原文)</th>
              <th className="p-4 text-left font-black w-[30%]">日本語 (対訳)</th>
              <th className="p-4 text-left font-black w-[40%]">日本への影響とリスク</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, idx) => (
              <tr key={idx} className="border-b border-emerald-100 hover:bg-emerald-50/50 transition-colors">
                <td className="p-4 align-top font-black text-emerald-700 bg-emerald-50/30">{row.id}</td>
                <td className="p-4 align-top font-medium text-slate-600 break-words whitespace-pre-wrap">{row.cn}</td>
                <td className="p-4 align-top font-bold text-slate-900 break-words leading-relaxed">
                  {row.jp ? renderMarkdownText(row.jp, `jp-${idx}`) : (
                ...
                <td className="p-4 align-top font-bold text-slate-900 break-words leading-relaxed">
                  {row.impact ? renderMarkdownText(row.impact, `imp-${idx}`) : (
...
const AnalysisProgressBar: React.FC<{ progress: number; status: string }> = ({ progress, status }) => (
  <div className="w-full max-w-4xl mx-auto mb-12 animate-in fade-in zoom-in duration-500">
    <div className="flex justify-between items-end mb-4">
      <div>
        <p className="text-[10px] font-black text-green-400 uppercase tracking-[0.3em] mb-1">Integrated Scrutiny Engine</p>
        <div className="flex items-center gap-3">
          <Search className="text-green-500 animate-pulse" size={20} />
          <h3 className="text-xl font-black text-white truncate max-w-[300px]">{status}</h3>
        </div>
      </div>
      <div className="text-right">
        <span className="text-3xl font-black text-green-500 italic">{progress}%</span>
      </div>
    </div>
...
【ID規則】質問は Q1, Q2...。回答は A1-1, A1-2...。冒頭発言（P等）は除外せよ。`;
      ...
      // Step 3: 逐次解析
      setStatusMsg('対訳および影響度分析を順次実行中...');
      const batchSize = 3; 
      for (let i = 0; i < structure.length; i += batchSize) {
        const batch = structure.slice(i, i + batchSize);
        const currentProgress = 30 + Math.floor((i / structure.length) * 70);
        setProgress(currentProgress);

        const fillPrompt = `以下の各項目を精密に日本語に対訳し、「日本への影響とリスク」を分析せよ。

■ 翻訳ルール：
1. 固有名詞「委内瑞拉」は必ず「ベネズエラ」と翻訳せよ。

■ 分析ルール：
1. 「日本への影響とリスク」の冒頭に、必ず影響度を [赤]【影響度：高/中/低/無】[/赤] の形式で記載せよ。
2. その後、日本の主権、安全保障等への具体的な影響をOSDSHの基準で記述せよ。
...
${JSON.stringify(batch)}`;
  ...
  const handleCopyReportCode = () => {
    const tableMarkdown = `| ID | 中国語 (原文) | 日本語 (対訳) | 日本への影響とリスク |\n|:---:|:---|:---|:---|\n` + 
      tableRows.map(r => `| ${r.id} | ${r.cn} | ${r.jp || "未解析"} | ${r.impact || "未解析"} |`).join('\n');
...
${tableMarkdown}
  ...
  const handleWordExport = () => {
    const cleanForWord = (text: string | undefined) => {
      if (!text) return "";
      return text.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>').replace(/\[赤\](.*?)\[\/赤\]/g, '<span style="color:#ff0000; font-weight:bold;">$1</span>');
    };
...
<body>
  ...
  const handleZohoHtmlExport = () => {
    const cleanForHtml = (text: string | undefined) => {
        if (!text) return "";
        return text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\[赤\](.*?)\[\/赤\]/g, '<span style="color:red; font-weight:bold;">$1</span>');
    };
  ...
  const ActionButtons: React.FC<{ className?: string }> = ({ className = "" }) => (
    <div className={`flex flex-wrap items-center gap-3 ${className}`}>
        <button onClick={handleWordExport} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-black shadow-lg transition-all text-sm active:scale-95">
        ...
        <button onClick={handleCopyReportCode} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-black shadow-lg transition-all text-sm border border-white/20 active:scale-95 ${copyStatus ? 'bg-green-600 text-white' : 'bg-slate-700 text-white hover:bg-slate-600'}`}>
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 pb-8 border-b border-emerald-900">
          <div className="flex items-center gap-6">
            <div className="bg-white p-2 rounded-2xl shadow-2xl relative border-2 border-green-500/30 shrink-0">
              ...
              <div className="absolute -bottom-1 -right-1 bg-green-600 text-[8px] font-black px-1 rounded border border-white shadow-sm">OSDSH</div>
            </div>
            <div>
              <p className="text-xs font-black text-emerald-500 tracking-widest uppercase">Unified Scrutiny (Type B)</p>
              <div className="flex items-baseline gap-3">
                <h1 className="text-3xl font-black text-white tracking-tighter uppercase leading-none">中国複合法律戦 統合対訳解析システム</h1>
                <span className="text-[10px] font-black bg-green-600 text-white px-1.5 py-0.5 rounded shadow-sm border border-white/20">{SYSTEM_VERSION}</span>
              </div>
            </div>
          </div>
          {showReport && !loading && <ActionButtons />}
          {showReport && (
            <button onClick={() => { setShowReport(false); setTableRows([]); setProgress(0); }} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-6 py-3 rounded-2xl font-black transition-all text-sm border border-slate-600 shadow-lg">
        ...
        {!showReport && !loading && (
          <section className="bg-emerald-950/40 p-6 md:p-12 rounded-[2.5rem] shadow-2xl border border-emerald-900 backdrop-blur-xl animate-in fade-in duration-500">
            <div className="mb-8 p-6 bg-green-950/40 border-l-[8px] border-green-600 rounded-r-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3 text-white">
                  ...
                  <span className="font-black text-green-400 text-xl uppercase tracking-tight">タイプB：統合的証拠化とナラティブ解体</span>
                </div>
                <p className="text-emerald-200 font-medium text-sm leading-relaxed">会見文を全件ID化し、対訳と「日本への影響度（赤字表示）」を網羅した統合レポートを生成します。</p>
              </div>
              <a href="https://www.mfa.gov.cn/fyrbt_673021/jzhsl_673025/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-green-600 hover:bg-green-500 text-white px-8 py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all border border-green-400">
                <Globe size={24} /> <span className="text-lg font-bold">中国外交部公式サイト</span>
              </a>
            </div>
            <textarea 
            ...
            />
            <button onClick={runAnalysis} disabled={!val.trim()} className="w-full mt-8 py-6 rounded-[2rem] font-black text-xl bg-gradient-to-br from-green-600 to-green-800 text-white active:scale-95 transition-all flex items-center justify-center gap-4 border border-green-400/30 shadow-2xl px-12">
        ...
        {loading && !showReport && (
          <div className="text-center py-40 flex flex-col items-center justify-center animate-in zoom-in duration-300">
            ...
            <p className="text-3xl font-black text-green-500 tracking-[0.2em] uppercase italic animate-pulse">Analyzing Integrity...</p>
          </div>
        )}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20">
            ...
            <div className="bg-[#fcfcfc] p-6 md:p-16 rounded-[2.5rem] shadow-4xl text-slate-900 border border-emerald-200">
              <div className="mb-12 border-b-2 border-slate-100 pb-8">
                <h2 className="text-3xl font-black tracking-tighter text-slate-900 uppercase">中国複合法律戦 対訳＆個別影響分析 統合レポート (Type B)</h2>
                <p className="text-emerald-600 font-bold text-xs uppercase tracking-widest mt-2">OSDSH INTEGRATED SECURITY SCRUTINY: {dt}</p>
              </div>
              
              <div className="mb-16">
                <h2 className="text-2xl font-black text-emerald-800 border-l-[10px] border-emerald-800 pl-4 mb-8 flex items-center gap-3"><InfoIcon /> 会見情報の確定</h2>
                <div className="p-8 bg-emerald-50 border-2 border-emerald-100 rounded-3xl mb-12 shadow-sm">
                  <h3 className="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-3 underline decoration-emerald-200 underline-offset-4">【翻訳分析対象の情報名】</h3>
                  {progress < 15 ? <div className="animate-pulse h-4 bg-emerald-100 rounded w-full"></div> : <div className="text-slate-800 font-black text-lg leading-relaxed">{renderMarkdownText(infoName, "h-info")}</div>}
                </div>
